...
import com.hopeonllc.askmom_user_service.response.ApiErrorResponse;
...
public class GlobalExceptionHandler {
     ...
     * Request DTO（如 UserRequest）上的约束注解校验失败，
     * 例如 @Email、@Size、@NotBlank。
     *
     * 注意：它不覆盖所有参数校验失败场景。
    ...
    public ResponseEntity<ApiErrorResponse> handleMethodArgumentNotValidException(
        ...
        // 把每个字段错误变成文本，然后用 ; 拼起来。
        // 例如可能得到：email: must not be blank; password: size must be between 8 and 20
    ...
    public ResponseEntity<ApiErrorResponse> handleBusinessException(
        ...
        // 先根据 错误的实例找到对应 的错误码，然后映射到 HTTP 状态码
    ...
    private HttpStatus mapBusinessStatus(BusinessException ex) {
        ...
        if (ex instanceof UserProfileAlreadyExistsException) {
            return HttpStatus.CONFLICT;
        }
        return HttpStatus.BAD_REQUEST;
    }
