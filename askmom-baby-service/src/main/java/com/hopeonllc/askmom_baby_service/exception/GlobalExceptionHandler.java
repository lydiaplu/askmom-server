...
import com.hopeonllc.askmom_baby_service.response.ApiErrorResponse;
...
public class GlobalExceptionHandler {
     ...
     * Request DTO（如 BabyRequest）上的约束注解校验失败，
     * 例如 @Size、@NotNull、@Pattern。
     *
     * 注意：它不覆盖所有参数校验失败场景。
    ...
    public ResponseEntity<ApiErrorResponse> handleMethodArgumentNotValidException(
        ...
        // 把每个字段错误变成文本，然后用 ; 拼起来。
        // 例如可能得到：userId: must not be null; feedingType: must match allowed values
    ...
    private HttpStatus mapBusinessStatus(BusinessException ex) {
        ...
        if (ex instanceof BabyAlreadyExistsException) {
